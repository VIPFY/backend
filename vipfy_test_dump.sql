...
COPY public.address_data (id, unitid, country, address, description, priority, tags) FROM stdin;
...
COPY public.email_data (unitid, email, verified, autogenerated, description, priority, tags) FROM stdin;
...
COPY public.message_data (id, receiver, sender, sendtime, readtime, archivetimesender, archivetimereceiver, messagetext, tags) FROM stdin;
...
95	1	67	23	2018-01-14 15:17:48.004794	\N	f	f	Neuer tags, neues Glück
96	1	67	23	2018-01-14 15:27:03.647833	\N	f	f	Neuer tags, neuer Versuch
...
COPY public.phone_data (unitid, number, verified, autogenerated, description, priority, tags) FROM stdin;
\.
...
COPY public.website_data (website, unitid, tags, verified, autogenerated, description, priority) FROM stdin;
\.
...
CREATE RULE "_RETURN" AS
    ...
    ( SELECT website_data.website
          ...
          WHERE ((website_data.unitid = u.id) AND (website_data.tags = 'SUPPORT'::text))
          ORDER BY website_data.priority
         LIMIT 1) AS supportwebsite,
    ( SELECT phone_data.number
           FROM public.phone_data
          WHERE ((phone_data.unitid = u.id) AND (phone_data.tags = 'SUPPORT'::text))
