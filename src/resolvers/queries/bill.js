  ...
  fetchPaymentData: requiresRights(["view-paymentdata"]).createResolver(
    async (parent, args, { models, session }) => {
      try {
        ...
        let stripeCards = await listCards(paymentData.payingoptions.stripe.id);

        const sortedcards = [];
        if (paymentData.payingoptions.stripe.cards) {
          paymentData.payingoptions.stripe.cards.forEach(c => {
