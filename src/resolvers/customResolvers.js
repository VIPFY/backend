...
export const find = async data => {
  try {
    const searches = {};
    await Object.keys(data).forEach(search => {
      searches[search] = (parent, args, { models }, info) => {
        if (data[search] == "Human" || data[search] == "Department") {
    ...
    throw new NormalError({
      message: err.message,
      internalData: { error: "Something went wrong with a custom resolver", data }
    });
  }
};
...
export const findCompany = () => ({
  company: (parent, args, { models }) => {
    console.log(parent.dataValues.company);
    return models.Department.findOne({
      where: { unitid: parent.dataValues.company }
    });
  }
});
