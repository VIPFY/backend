            ...
            case "11":
              {
                ...
                if (checkDomain.code != 200) {
                  ...
                  const hasAccount = await models.BoughtPlan.findOne({
                    where: { planid: [25, 48, 51, 50, 49] },
                    attributes: ["id"],
                    raw: true
                  });
                  ...
                  if (hasAccount) {
                    const accountData = await models.Licence.findOne({
                      where: {
                        boughtplanid: hasAccount.id
                      },
                      attributes: ["key"]
                    });
                    options.cid = accountData.key.cid;
